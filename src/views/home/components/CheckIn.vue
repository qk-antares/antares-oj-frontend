...
<template>
  ...
  <div class="mx-[10px] grid grid-cols-7 text-xs">
    <span
      v-for="day in week"
      :key="day"
      class="text-gray-400 w-8 flex items-center justify-center h-[26px] pointer-events-none"
      >{{ day }}</span
    >
    <span
      v-for="i in Array.from({ length: startDay || 0 }, (_, index) => index)"
      :key="i"
    ></span>
    <a
      v-for="i in Array.from({ length: totalDay || 0 }, (_, index) => index)"
      :key="i"
      class="text-slate-600 relative w-8 h-8 flex cursor-pointer items-center justify-center rounded-full hover:bg-gray-200"
    >
      <span>{{ i + 1 }}</span>
      <span
        class="absolute bottom-[3px] h-1 w-1 rounded-full bg-red-500"
      ></span>
    </a>
  </div>
</template>
